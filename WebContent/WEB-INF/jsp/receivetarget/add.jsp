...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container">
    ...
    <div class="span9 margin-top-one">
      <div class="row-fluid">
        <h3 style="text-align: center;">接待对象新增  New reception object</h3>
      </div>
    </div>
    ...
    <form action="${ctx}/ReceiveTarget/add.do" method="post" onsubmit="return verify()">
	    <div class="span12">
	      <div class="row-fluid">
			  <div class="span5 ">
			   	  <label class="labelroomnumber">对象类别：<br/><span style="font-size: 12px;">Object class</span></label>
			      <select name="targetTypeID" class="cboone inputone">
		            <c:forEach items="${listOne}" var="item">
						<c:if test="${item.far_id==56}">
							<option value="${item.far_id}" <c:if test="${item.far_id==56}" >selected="selected"</c:if>>
									${item.attributeDetailsName}team
							</option>
						</c:if>
						<c:if test="${item.far_id==55}">
							<option value="${item.far_id}">
									${item.attributeDetailsName}Individual travelers
							</option>
						</c:if>
			        </c:forEach> 
		          </select>
			  ...
			  <div class="span5">
			   	  <label class="labelroomnumber" style="margin-right:16px;">负责人：<br/><span style="font-size: 12px;">Person in charge</span></label>
	    ...
	    <div class="span12">
	      <div class="row-fluid">
			  ...
			  <div class="span5">
			   	  <label class="labelroomnumber">团队编号：：<br/><span style="font-size: 12px;">Team number</span></label>
	    ...
	    <div class="span12">
	      <div class="row-fluid">
	        <div class="span5">
	           <label class="labelroomnumber">登记时间：：<br/><span style="font-size: 12px;">Registration time</span></label>
		    ...
		    <div class="span5">
			   	<label class="labelroomnumber">联系电话：：<br/><span style="font-size: 12px;">Contact number</span></label>
 ...
 <script type="text/javascript">
    ...
    if(document.getElementById("principalId").value==""){
	     ...
	     alert("你输入的    负责人   太过长了  请不要超出  10 位长度 "+' \n '+"The person in charge you entered is too long");
	     document.getElementById("principalId").focus();
	     return false;
     ...
     if(value!=""){
       $.ajax({                                                      
          ...
          success: function (result) {                          // 不出现异常 进行立面方
              if(result>=1){                                  
                   alert("已存在此团队编号，请另取团队编号！"+' \n '+"This team number already exists");                     //提示框
