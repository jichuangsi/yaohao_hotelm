...
<div class="container" >
	<div class="span5">
		<div class="row-fluid">
			<label class="labelroomnumber">供应商名称：<br/><span style="font-size: 12px;">Supplier name</span> </label>
			<form action="${ctx}/Supplier/tolist.do" method="post" style="float: left;">
	...
	<div class="dgvone">
		<table class="table table-condensed table-bordered table-striped" id="tableid">
			...
			<tr>
				...
				<th rowspan="2">是否启用<br/><span style="font-size: 12px;">Is it enabled</span></th>
				...
				<tr>
					...
					<c:if test="${item.isenabled==0}">
						<td><a style="color:#00be67;margin-right: 30px">已启用<span style="font-size: 12px;">Enabled</span></a>
							<a style="color: red"href="${ctx}/Supplier/delete.do?id=${item.id}">禁用
								<span style="font-size: 12px;">Prohibit</span></a></td>

					</c:if>
					<c:if test="${item.isenabled==1}">
						<td><a style="color: #00be67;margin-right: 30px">已禁用<span style="font-size: 12px;">disabled</span></a>
							<a style="color: red"href="${ctx}/Supplier/enabled.do?id=${item.id}">启用
								<span style="font-size: 12px;">Enable</span></a></td>
...
<script type="text/javascript">
    /*$(document).ready(function (){
        ...
        alert(is);
        if (is==1) {
            alert("因改供应商还有订单未处理无法禁用！");
		}
	});*/
    function addfunction(){
        parent.document.getElementById('Mainid').src='${ctx}/Supplier/toadd.do';
    }
    ...
    function updatefunction(){
        ...
        if(chk_value!=""){
            if(chk_value.toString().indexOf(",")>0){
                ...
                parent.document.getElementById("Mainid").src='${ctx}/Supplier/toupdate.do?id='+chk_value;
            }
        }else{
    ...
    function deletefunction(){
        ...
        if(chk_value!=""){
            ...
            if(flag){
                parent.document.getElementById("Mainid").src='${ctx}/Supplier/delete.do?id='+chk_value;
            }
        }else{
