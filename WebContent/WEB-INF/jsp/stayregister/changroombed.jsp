...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container" style="height:630px;overflow-x:auto;">
    ...
    <div class="span9 margin-top-one">
      <div class="row-fluid">
        <h3 style="text-align: center;">更换房间(Change rooms)</h3>
    ...
    <input id="LvKeLeiXingId" type="hidden" value="${LvKeLeiXingId}">
		<input id="supplier" type="hidden" value="${supplierid}">
		  ...
		  <div class="span4" style="text-align:right;">
		      <div class="row-fluid">
			      <div class="span10" style="margin-bottom: 8px;">
				   	 ...
				   	 <li class="icon-check icon-white"></li>确定更换房间(Confirm room change)</button>
				  </div> 
			  </div>
	      </div>
		    ...
		    <div class="row-fluid">
			  <div class="span10"  style="margin-bottom: 8px;"> 
		        <button style="margin-top:2px;" href="#xuanzhe" data-toggle="modal" onclick="selectRoom()"
		         class="btn btn-info"><li class="icon-plus icon-white"></li>选择房间(Select room)</button>
              </div>
            </div>
          </div>
    ...
    <!--  ———————————————————————————————————————————————————————————————————————————————————————— -->
	    <div class="span12">
	      <div class="row-fluid">
		     <div class="span3">
		        <label>原房间号：<span style="font-size: 12px;">Original room number</span></label>
		        <input type="text" style="width:100%;height:27px;" readonly="readonly" value="${list[0].roomNumber}"> 
		     </div>
		     <div class="span3">
		        <label>新房间号：<span style="font-size: 12px;">New room number</span></label>
		        <input id="newRoomNumberId" type="text" style="width:100%;height:27px;" readonly="readonly"> 
		      </div>
		     <div class="span3">
		        <label>新房价/天：<span style="font-size: 12px;">New housing price/day</span></label>
		        <input id="roomPriceId" type="text" style="width:100%;height:27px;" readonly="readonly"> 
		     </div>
		     <div class="span3">
		        <label>换房时间：<span style="font-size: 12px;">Room change time</span></label>
      ...
      <div class="span12 ">
      <fieldset>
        <legend>原房间信息：<span style="font-size: 12px;">Original room information</span></legend>
      </fieldset>
      </div>
      ...
      <div class="span12">
	   ...
	   <div class="span12">
	      <div class="row-fluid">
		     <div class="span3">
		        <label>登记时间：<span style="font-size: 12px;">Registration time</span></label>
		        <input type="text" style="width:100%;height:27px;" readonly="readonly" value="${list[0].registerTime}"> 
		     </div>
		     <div class="span3">
		        <label>房价/天：<span style="font-size: 12px;">housing price/day</span></label>
		        <input type="text" style="width:100%;height:27px;" readonly="readonly" value="${list[0].bedDayPrice}">
		      </div>
			  <div class="span3">
				  ...
				  <input id="checkintimeId" name="checkintime"  style="width:97%; height:27px;float:left;" class="Wdate" type="text"
						 onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd',onpicked:pickedFunc})" onchange="onchangeOne()" value="${list[0].checkintime}"/>
			  ...
			  <div class="span3">
				  ...
				  <input id="checkouttimeId" name="checkouttime"  style="width:97%; height:27px;float:left;" class="Wdate" type="text"
						 onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd',onpicked:pickedFunc})" onchange="onchangeOne()"value="${list[0].checkouttime}"/>
	     ...
	     <div class="span12">
	      <div class="row-fluid">
			  <div class="span3">
				  ...
				  <input id="supId" type="text" style="width:100%;height:27px;"readonly="readonly"  >
			  </div>
		  </div>
	    </div> 
     ...
     <div class="modal hide fade" id="xuanzhe" style="margin-top:10px;">
       <div class="span5" style="width:98%;height:480px; overflow-x:auto;">
	    <div class="row-fluid">
		   <div class="span8">
		      <label class="labelroomnumber">房间号：<span style="font-size: 12px;">Room Number</span></label>
			   ...
			   <div class="input-append">
			      <button onclick="selectRoom()" class="btn-success textone" style="margin-left:-4px;height:27px;"><li class="icon-search icon-white"></li>search</button>
			   </div>
	       </div>
	       <div class="span4">
	          <button data-dismiss="modal" class="btn btn-info btn-small textone" type="button" onclick="changroomConfirmfunction()">
	        ...
	        <c:forEach items="" var="item">
		        <tr>
		          ...
		          <td>${item.roomAmount}</td>
					<td>${item.surplusAcount}</td>
		          <td>￥${item.bedDayPrice}</td>
		        </tr>
	        </c:forEach>
	      </tbody>
	    </table>
 ...
 <script type="text/javascript">
    ...
    var supplier=document.getElementById("supplier").value;//"&isbed="+isbed+
		 var stayId=document.getElementById("stayId").value;
    ...
    $.ajax({                                                      
          ...
          url: '${ctx}/StayRegister/changRoomSelectPassengerBed.do',       //地址 type 带参数
          data:"roomNumber="+name+"&supplier="+supplier+"&stayId="+stayId,                         // IDCardValue 自定义的。相当于name把值赋予给 他可以在servlet 获取
//        dataType:"json",                                       // json 数据类型提交 
          ...
          success: function (result) {  
            for (var key in result) { 
                ...
                var tdsurplusAcount = tr.insertCell(-1);
                ...
                tdsurplusAcount.innerHTML =item.surplusAcount;


            }
            if(i==0){
              alert("很抱歉！没有可替换的房间"+' \n '+"I'm sorry! No room to replace");
            }               
          },
          error: function(data) {
   ...
   function changroomConfirmfunction(){
	 ...
	 /*   roomPrice=table.rows[selectedIndex-1].cells[5].innerHTML;*/
        ...
        bedPrice=table.rows[selectedIndex-1].cells[4].innerHTML;
        supId=table.rows[selectedIndex-1].cells[5].innerHTML;
	  });
	  if(chk_value!=""){
        document.getElementById("roomId").value=chk_value;
        document.getElementById("newRoomNumberId").value=roomNumber;
            document.getElementById("roomPriceId").value=bedPrice;//床位出租\
            document.getElementById("supId").value=supId;//剩余床位数
	  }else{
	  }
   }
   ...
   function confirmChangRoom(){     //确定更换房间
     if(document.getElementById("newRoomNumberId").value==""){
       alert("请选择要更换到的房间！"+' \n '+"Please select the room to change to");
       return false;
     }
     ...
     var LvKeLeiXingId=document.getElementById("LvKeLeiXingId").value;
     parent.document.getElementById('Mainid').src='${ctx}/StayRegister/confirmChangRoomBed.do?id='+id+
