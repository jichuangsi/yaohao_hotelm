...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <input type="hidden" id="isBillID" value="${list[0].isBillID}"/>
      ...
      <div class="row-fluid">
		...
		<div class="space4">
			<form action="" method="post" id="form2">
				<input name="depID" type="hidden" value="${stayId}">
		<select id="teamIsBill" style="width:15%;height:27px; float:left; margin-right:5px;" name="depoist">
			<option value=" ">修改支付状态</option>
			<option value="0">已支付</option>
			<option value="1">待支付</option>
		</select>
				<div class="span1">
					<button class="btn btn-info btn-small" type="button" onclick="depoit()">
						<li class="icon-plus icon-white"></li>修改</button>
				</div>
	        ...
	        <c:forEach items="${list}" var="item">
		        <tr>
		          ...
		          <c:if test="${item.receiveTargetID!=2}">
					...
					<c:if test="${item.depoiststatus==0}">
						<td>已支付</td>
					</c:if>
					<c:if test="${item.depoiststatus==1}">
						<td>待支付</td>
					</c:if>
		        </tr>
	        </c:forEach>
    ...
    <div class="modal hide fade" id="lvke" style="text-align: center;width: 700px">
      <div class="span7" style="height:300px; overflow-x:auto;">
       <div class="span6" style="width:95.5%; background:#CEFFCE; margin-left:0px;" >
         <label>新增押金记录</label>
       </div>
	      ...
	      <div class="row-fluid">
		      ...
		      <div class="span3">
			      ...
			      style="width:97%; height:27px;float:left;" type="text">
			       <div id="divOne">
			         <label class="yansered" style="margin-top:50px;">*</label>
			       </div>
			  </div> 
			  <div class="span1"></div>
			  <div class="span3">
			   	  <label class="labelroomnumber">支付方式：</label>
			      <select name="depositPayWayID" class="cboone" style="width:50%;">
			  ...
			  <div class="span4">
				  <label class="labelroomnumber">押金状态：</label>
				  <select id="depoitstatus" class="cboone" style="width:35%;height:27px; float:left; " name="depoiststatus">
					  <option value=" ">修改支付状态</option>
					  <option value="0">已支付</option>
					  <option value="1">待支付</option>
				  </select>
 ...
 function tijiao(){
    ...
    function depoit(){
        if(document.getElementById("teamIsBill").value=="") {
            alert("请选择支付状态");
            return false;
        }
        var LvKeLeiXingId=document.getElementById("LvKeLeiXingId").value;
        form2.action="${cx}/StayRegister/updateDepoitStatus.do?LvKeLeiXingId="+LvKeLeiXingId;
        form2.submit();
	}
